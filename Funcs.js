...
function CreateButtons() {
    ...
    Player = new Button(Width / 12, Height / 2, PublicButtonRad/3);

    Fader_Btn = new Button(Width / 2, Height / 2, Fader_Btn_Rad);
    Fader_Btn.setRect(Width, PublicButtonRad * 2.2);
    ...
    Buttons.push(Logo_Btn);



    Score = new Button(Width / 2, Height / 3, PublicButtonRad);
    Score.setSqr();
    Score.setTxt("", true, color(Wrong_C), FontSize * 1.5);
    Score.Color = color(Bg_Color);
    Score.lerpRad = true;
    Score.lerpAlpha = true;
    Score.minRad = Score.maxRad;
...
function createScreenSizes() {
    ...
    Fader_Btn_Rad = Width / 3.5;
    PublicButtonRad = height / 4;
    Bottom = Height / 2 +PublicButtonRad/3 ;
    Top = Height / 2 - PublicButtonRad * 3;
}
...
function updatePlayer() {
    ...
    if ((Player.y + Player.minRad > Bottom)&&!JumpBool) {
        Player.update();
        return;
    }

    if ((Player.y - Player.minRad < Top && JumpBool)||SpeedY>=0 ) {
        JumpBool = false;
    }
